...
from .types import GetAvailabilityInfoResponse, GetTitleDetailsResponse, SearchResponse
...
class Client:
    ...
    def _search_request(
        ...
        search_items = []
        if keywords:
            search_items.append({"SearchField": "Keywords", "SearchTerms": keywords})
        if author:
            search_items.append({"SearchField": "Author", "SearchTerms": author})
        if subject:
            search_items.append({"SearchField": "Subject", "SearchTerms": subject})
        if title:
            search_items.append({"SearchField": "Title", "SearchTerms": title})
        if branch:
            search_items.append(
                {
                    "SearchField": "BranchID",
                    "SearchTerms": branch.value
            ...
            search_items.append(
                {
                    ...
                    "SearchTerms": media_code.value
                    if isinstance(media_code, MediaCode)
                    else media_code,
                }
            )
        if language:
        ...
        return {
            "APIKey": api_key,
            "SearchItems": {"SearchItem": search_items},
            "Modifiers": {
    ...
    def get_title_details(
        self, bid: str = None, isbn: str = None
        ...
        :return:
        """
        response = self._zeep.service.GetTitleDetails(
            APIKey=self.api_key, BID=bid, ISBN=isbn
        )
    ...
    def get_availability_info(
        ...
        :return:
        """
        response = self._zeep.service.GetAvailabilityInfo(
            APIKey=self.api_key,
            BID=bid,
            ISBN=isbn,
            Modifiers={
